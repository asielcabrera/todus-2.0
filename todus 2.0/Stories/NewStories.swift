...
//  todus 2.0
//
//  Created by Asiel Cabrera on 9/24/20.
...
struct NewStories: View {
    ...
    func content() -> some View {
        return StoryView(imageNames: ["image01","image02","image03","image04","image05","image06","image07"], showStory: .constant(false))
    }
    ...
    func wave(data: Binding<SliderData>) -> some View {
        ...
        return WaveView(data: data.wrappedValue).gesture(gesture)
            .foregroundColor(Config.colors[index(of: data.wrappedValue)].opacity(0.3))
    }
