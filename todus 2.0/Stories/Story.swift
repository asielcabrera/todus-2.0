...
struct StoryView: View {
    ...
    init(imageNames: [String], showStory: Binding<Bool>) {
        self.imageNames = imageNames
        self._showStory = showStory
        self.storyTimer = StoryTimer(items: self.imageNames.count, interval: 3.0, showStory: showStory)
...
struct StoryView_Previews: PreviewProvider {
    static var previews: some View {
        StoryView(imageNames: ["image01","image02","image03","image04","image05","image06","image07"], showStory: .constant(false))
    }
}
...
class StoryTimer: ObservableObject {
    ...
    init(items: Int, interval: TimeInterval, showStory: Binding<Bool>) {
        self.max = items
        self.progress = 0
